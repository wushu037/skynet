...
int snlua_init(struct snlua *l, struct skynet_context *ctx, const char *args) {
    ...
    uint32_t handle_id = strtoul(self + 1, NULL, 16); // 将字符串转换成 unsigned long (无符号长整型数)
    // it must be first message
    skynet_send(ctx, 0, handle_id, PTYPE_TAG_DONTCOPY, 0, tmp, sz); // 倒数第三个参数为0 表示session
    return 0;
}
...
struct snlua *
snlua_create(void) {
    struct snlua *l = skynet_malloc(sizeof(*l)); // 创建一个snlua实例
    memset(l, 0, sizeof(*l));
    l->mem_report = MEMORY_WARNING_REPORT; // 32M
    l->mem_limit = 0;
    l->L = lua_newstate(lalloc, l); // 创建一个lua虚拟机
    l->activeL = NULL;
    ATOM_INIT(&l->trap, 0);
    return l;
}
