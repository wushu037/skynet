...
#define CHECKCALLING_BEGIN(ctx) \
    if (!(spinlock_trylock(&ctx->calling))) { assert(0); }
...
skynet_context_new(const char *name, const char *param) {
    struct skynet_module *mod = skynet_module_query(name); // 通过name获取模块

    if (mod == NULL)
        return NULL;

    void *inst = skynet_module_instance_create(mod); // 根据模块创建对应的实例化对象
    if (inst == NULL)
        return NULL;
    ...
    ctx->handle = 0;                                                          // 不设置的话 是一个随机值
    ctx->handle = skynet_handle_register(ctx);                                // handle就是服务的地址。每个服务都会分配一个唯一的handle，每个服务的队列也会保存这个handle。skynet_handle_register就是给去注册服务并且获取分配的handle
    struct message_queue *queue = ctx->queue = skynet_mq_create(ctx->handle); // 创建服务的消息队列
    // init function maybe use ctx->handle, so it must init at last
    context_inc(); // 服务总数加一个

    CHECKCALLING_BEGIN(ctx)
    int r = skynet_module_instance_init(mod, inst, ctx, param); // 对模块的实例进行初始化
    ...
    if (r == 0) {
        ...
        skynet_globalmq_push(queue); // 把服务队列加入全局队列
        if (ret) {
...
// 向服务中push消息：主要是通过服务的id得到服务，然后把消息加入到服务对应的队列中
...
skynet_context_message_dispatch(struct skynet_monitor *sm, struct message_queue *q, int weight) {
    ...
    uint32_t handle = skynet_mq_handle(q); // 通过队列获取他对应的handle(服务地址)

    struct skynet_context *ctx = skynet_handle_grab(handle); // 通过handle获取ctx(服务)
    if (ctx == NULL) {
        struct drop_t d = {handle};
    ...
    for (i = 0; i < n; i++) {
        ...
        if (ctx->cb == NULL) {
            ...
            dispatch_message(ctx, &msg); // next，把消息交给服务内部的回调函数处理
        }
